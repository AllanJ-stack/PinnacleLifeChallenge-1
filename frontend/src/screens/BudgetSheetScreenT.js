...
export default function BudgetSheetScreen() {
  ...
  const getBudgetDetails = async () => {
    ...
    for (let prevYear of prevYearsArr) {
      ...
      const currYearSalaryRes = await Axios.get(
        `/companies/${userCompany}/salary/${prevYear}.json`
      );
      ...
      if (userRecords.sell_trans[prevYear]) {
        for (let [productId, productRecord] of Object.entries(
          userRecords.sell_trans[prevYear]
        )) {
          ...
          const productCommissionRes = await Axios.get(
            `companies/${userCompany}/commission/${prevYear}/${productId}.json`
          );
          ...
          for (let [userId, qty] of Object.entries(productRecord)) {
            // To get the total quantity sold for this product
            totalProductQty += qty;
            
          }
          console.log(
            `Income: The total quantity of product(${productId}) sold is ${totalProductQty}`
          );
          // To get the total commision of this item
          let totalProductCommission = productCommission * totalProductQty;
          console.log(
            `Total Commision of ${productId} is ${totalProductCommission}`
          );
      ...
      let totalExpense = 0;
      console.log(userRecords.buy_trans);

      if (userRecords.buy_trans[prevYear]) {
        for (let [productId, productRecord] of Object.entries(
          userRecords.buy_trans[prevYear]
        )) {
          ...
          const productCostRes = await Axios.get(
            `product/${userCompany}/${prevYear}/${productId}/priceValue.json`
          );
          ...
          let totalProductQty = 0;
          for (let [userId, qty] of Object.entries(productRecord)) {
            // To get the total quantity bought for this product
            totalProductQty += qty;
            
          }
          console.log(
            `Expense: The total quantity of product(${productId}) bought is ${totalProductQty}`
          );
          // To get the Total Expense of this item
          let totalProductExpense = productCost * totalProductQty;
          console.log(
            `Total Expense of ${productId} is ${totalProductExpense}`
          );
  ...
  useEffect(() => {
     if (!budgetDetails) {
    getBudgetDetails().then((budgetDetails) => {
